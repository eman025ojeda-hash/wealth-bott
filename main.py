...
# ── IN-MEMORY STORAGE ──────────────────────────────────────────
# Expenses stored in memory (persists while bot is running)
EXPENSES = []
COUNTER  = [0]  # mutable so we can increment inside functions
...
async def parse_text_expense(text):
    prompt = f"""Extract expense from this message (Filipino/English, Philippine Peso currency):
"{text}"
Reply ONLY in JSON format: {{"name":"store name","amount":123.45,"category":"Food/Transport/Utilities/Shopping/Entertainment/Healthcare/Other","note":"brief note"}}
If NOT an expense reply exactly: NOT_EXPENSE"""
    try:
...
async def parse_receipt_image(image_bytes):
    ...
    prompt = """Read this receipt photo. Currency is Philippine Peso.
Reply ONLY in JSON: {"name":"merchant name","amount":123.45,"category":"Food/Transport/Utilities/Shopping/Entertainment/Healthcare/Other","note":"items summary"}
If unreadable reply exactly: CANNOT_READ"""
    try:
        contents = [{"parts": [
...
async def expenses_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not EXPENSES:
        await update.message.reply_text("Wala pang expenses!\n\nType: `spent ₱250 Jollibee` o mag-send ng receipt photo 📸", parse_mode="Markdown")
        return
...
async def today_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not today_exp:
        await update.message.reply_text(f"Wala pang expenses today!"); return
...
async def delete_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage: /delete [id]\nHalimbawa: /delete 5\n\nUse /expenses to see IDs."); return
    try:
        ...
        if to_remove:
            ...
            await update.message.reply_text(f"✅ Expense #{exp_id} *{to_remove['name']}* deleted na!", parse_mode="Markdown")
        else:
            await update.message.reply_text(f"❌ Expense #{exp_id} not found. Use /expenses to see IDs.")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ May error. Try ulit!")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not result:
        await update.message.reply_text(
            "🤔 Hindi ko nakuha yan. Try:\n\n"
            "• `spent ₱250 Jollibee`\n"
            "• `bayad ₱1500 Meralco`\n"
            "• `groceries ₱2300`\n"
            "• O mag-send ng 📸 receipt photo!", parse_mode="Markdown")
        return
    entry = add_expense(result["name"], result["amount"], result.get("category","Other"), result.get("note",""))
    await update.message.reply_text(
        f"✅ *Naitala na!*\n\n"
...
def main():
    ...
    logger.info("🚀 Wealth+ Bot is running!")
